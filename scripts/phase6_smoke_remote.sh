...
OUTPUT_DIR="$RUNTIME_DIR/phase6-field-smoke-$(date +%Y%m%d-%H%M%S)"
...
if [[ "$active_sessions" != "0" || "$active_jobs" != "0" || "$chromium_processes" != "0" ]]; then
  printf 'Refusing Phase 6 smoke while browser work is active.\n' >&2
  exit 20
fi
...
cd "$APP_DIR"
"$RUNTIME_DIR/.venv/bin/python" scripts/smoke_meta_field_filling.py \
...
if pgrep -c chromium >/dev/null; then
  printf 'Chromium remained after Phase 6 smoke.\n' >&2
  exit 21
fi
printf 'output_dir=%s\n' "$OUTPUT_DIR"
