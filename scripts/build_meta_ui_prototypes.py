...
<head>
  ...
  <style>
    ...
    * { box-sizing: border-box; }
    ...
    .dialog-body { max-height: calc(100vh - 210px); overflow: auto; padding: 20px; }
  ...
  <dialog id="campaign-dialog">
    <form data-prototype-form="campaign">
      ...
      <div class="dialog-body">
        <div class="dialog-fields"><div class="field"><label for="campaign-account">Ad account<span class="required-mark" aria-hidden="true">*</span></label><select id="campaign-account" required><option>Lush Ads Main · USD</option><option>Campaign Lab · VND</option></select><div class="field-help">Resource và asset sẽ được lọc theo ad account này.</div></div><div class="field"><label for="campaign-name">Tên campaign<span class="required-mark" aria-hidden="true">*</span></label><input id="campaign-name" value="Awareness — August Launch" required></div><div class="field-row"><div class="field"><label for="campaign-objective">Objective<span class="required-mark" aria-hidden="true">*</span></label><select id="campaign-objective" required><option>Awareness</option><option>Traffic</option><option>Engagement</option><option>Leads</option><option>App promotion</option><option>Sales</option></select></div><div class="field"><label for="campaign-budget">Ngân sách mỗi ngày (USD)<span class="required-mark" aria-hidden="true">*</span></label><input id="campaign-budget" inputmode="decimal" value="50.00" required></div></div><div class="dialog-inline-meta">Bản nháp sẽ dừng ở Review để bạn kiểm tra trước khi tiếp tục.</div><div class="field"><label for="campaign-note">Ghi chú (không bắt buộc)</label><textarea id="campaign-note" placeholder="Ví dụ: Kiểm tra nội dung và ngân sách trước khi tiếp tục."></textarea></div></div>
      </div>
      ...
      document.querySelectorAll('[data-dialog-open]').forEach(function (button) {
        button.addEventListener('click', function () {
          ...
          if (dialog && !dialog.open) {
            dialog.showModal();
            const preferredFocus = dialog.querySelector('input:not([readonly]), select:not([disabled]), textarea:not([readonly])')
