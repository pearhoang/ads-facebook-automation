...
if [[ "$active_sessions" != "0" || "$active_jobs" != "0" || "$chromium_processes" != "0" ]]; then
  printf 'Refusing Phase 8 backup while browser work is active.\n' >&2
  exit 20
fi

mkdir -p "$RUNTIME_DIR/postgres-backups" "$RUNTIME_DIR/source-backups" "$RUNTIME_DIR/backups"
db_backup="$RUNTIME_DIR/postgres-backups/meta-ads-phase8-predeploy-$STAMP.dump"
source_backup="$RUNTIME_DIR/source-backups/meta-ads-phase8-predeploy-$STAMP.tar.gz"
env_backup="$RUNTIME_DIR/backups/worker.env-before-phase8-$STAMP"
...
install -m 0600 /etc/meta-ads-copilot/worker.env "$env_backup"
