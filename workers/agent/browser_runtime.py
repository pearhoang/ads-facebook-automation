...
def _is_snap_chromium(chromium_bin: str) -> bool:
    ...
    supplied = str(chromium_bin or "").strip().replace("\\", "/")
    if supplied == "/snap/bin/chromium" or supplied.startswith("/snap/chromium/"):
        return True
...
def _prefer_direct_snap_chromium(chromium_bin: str) -> str:
    """Bypass the snap launcher so ``--user-data-dir`` stays authoritative.

    The launcher at ``/snap/bin/chromium`` injects its shared
    ``$SNAP_USER_COMMON/chromium`` path into child processes. That leaks login
    cookies across otherwise distinct account profile keys. The packaged
    Chromium binary itself honors our explicit profile path.
    """
    if not _is_snap_chromium(chromium_bin):
        return chromium_bin
    direct = Path(
        os.getenv(
            "BROWSER_SESSION_SNAP_DIRECT_BIN",
...
class BrowserRuntimeManager:
    ...
    def load_config(self) -> BrowserRuntimeConfig:
        ...
        chromium_bin = _prefer_direct_snap_chromium(
            self._resolve_executable(list(dict.fromkeys(chromium_candidates)))
        )
        return BrowserRuntimeConfig(
            ...
            start_url=str(
                os.getenv(
                    "BROWSER_SESSION_START_URL",
                    "https://business.facebook.com/adsmanager/manage/campaigns",
                )
            ).strip()
            or "https://business.facebook.com/adsmanager/manage/campaigns",
            bind_host=str(os.getenv("BROWSER_SESSION_BIND_HOST", "127.0.0.1")).strip()
            or "127.0.0.1",
    ...
    @staticmethod
    def _build_novnc_url(base_url: str, session_id: str, access_password: str) -> str:
        encoded_password = quote(access_password, safe="")
        websocket_path = quote(f"browser/{session_id}/websockify", safe="")
        return (
            f"{base_url}/browser/{session_id}/vnc.html"
            f"?autoconnect=1&resize=scale&view_only=0&path={websocket_path}"
    ...
    def launch(self, record: dict[str, Any]) -> dict[str, Any]:
        ...
        if not start_url:
            start_url = "https://business.facebook.com/adsmanager/manage/campaigns"
        ...
        profile_deadline = time.time() + 6.0
        while time.time() < profile_deadline and not (profile_dir / "Local State").is_file():
            time.sleep(0.2)
        if not (profile_dir / "Local State").is_file():
            _terminate_pid(chromium.pid)
            _terminate_pid(openbox.pid)
            _terminate_pid(xvfb.pid)
            raise ValueError(
                "Chromium khong khoi tao dung profile path; "
                "kiem tra snap launcher va BROWSER_SESSION_CHROMIUM_BIN."
            )
    ...
    def save_session_record(self, record: dict[str, Any]) -> None:
        ...
        try:
            self._session_record_path(session_path).chmod(0o600)
        except OSError:
            pass
    ...
    def inspect(self, record: dict[str, Any]) -> dict[str, Any]:
        ...
        current = next(
            (
                ...
                if "facebook.com"
                in (urlparse(str(item.get("url") or "").strip()).hostname or "").lower()
            ),
