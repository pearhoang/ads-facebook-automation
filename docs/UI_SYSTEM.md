...
- Brand và footer sidebar dùng nền trắng trung tính để navigation là vùng điều hướng chính. Logo giữ tile xanh nhỏ; avatar Admin dùng nền xanh rất nhạt, không tạo thêm mảng màu cạnh tranh với selected navigation.
...
- `Meta Light Focus` tách vai trò màu để tăng scanability: logo/CTA/ad-account dùng Meta blue `#0866FF`; selected navigation là mảng Meta blue rõ với chữ/icon trắng; campaign draft dùng indigo; pending dùng amber; approved/resource dùng green. Inactive sidebar text dùng slate `#344054`; brand/footer trung tính để selected row là điểm neo duy nhất trong sidebar.
