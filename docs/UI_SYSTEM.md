...
- `/` là setup surface gộp Facebook profile, ad account và Meta resource; `/campaigns` là surface `Báo cáo quảng cáo`, không phải trình dựng campaign thay Ads Manager.
...
- Sidebar Ads chỉ có `Thiết lập tài khoản` tại `/` và `Báo cáo quảng cáo` tại `/campaigns`; route `/ad-accounts` và `/reports` redirect về anchor tương ứng để giữ bookmark cũ.
