...
- Sidebar uses solid white with a soft gray-blue border and slate `#344054` navigation text. Brand and Admin footer stay neutral; the selected row is the single strong navigation anchor with a solid Meta-blue surface and white text/icon.
- The blue monogram keeps a compact blue tile without a colored brand band. The Admin avatar uses a pale blue surface rather than a saturated gradient so the two sidebar ends do not compete with selection.
- Main canvas uses a near-white blue–violet gradient behind opaque white surfaces, neutral borders, shallow neutral shadows, and solid `#0866FF` primary actions. Panel headers drop decorative accent lines and icon tiles; dashboard helper copy is hidden while table/KPI typography and row spacing increase.
...
- Information hierarchy uses a restrained blue–indigo–amber–green system: blue for navigation/account/action, indigo for campaign drafts, amber for pending attention, and green for ready/approved/resource. KPI values and short bottom markers carry these roles; panel headers repeat only their relevant section icon color.
